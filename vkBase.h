...
namespace vulkan {
	...
	/// Vulkan API 返回的是 VkResult 错误码，而不是异常
	/// 忘记检查返回值，错误就会被忽略
...
//情况1：根据函数返回值确定是否抛异常
	...
	class result_t {
		VkResult _result;
	public:
		static std::function<void(VkResult)> callback_throw; // 静态全局的错误处理钩子，析构抛异常前会调用它
		result_t(VkResult result) : _result(result){} // 包装 vk 函数返回值
		result_t(result_t&& other) noexcept : _result(other._result) { other._result = VK_SUCCESS; } // 防止 other 在析构时抛异常，因为所有权已经转移
		// 若一个 result_t 对象在析构时仍保留错误码，说明这个错误没有被处理，于是抛出异常
		// 默认析构函数是 noexcept(true)，本来不允许抛异常；这里显式放开，避免错误被静默吞掉
		~result_t() noexcept(false) {
			if (uint32_t(_result) < VK_RESULT_MAX_ENUM) { // 如果不是错误码，什么都不做
				return;
			}
			if (callback_throw) { // 是错误码时，可以先交给回调处理
				callback_throw(_result);
			}
			throw _result; // 处理不了这个错误，就在对象生命周期结束时抛出
		}
...
#pragma warning(disable:6031)
	...
	public:
		...
		// 若 T 带 const 修饰，兼容从对应无 const 修饰版本的 arrayRef 构造
	...
	class graphicsBase {
		...
		std::vector<const char*> instanceLayers; // vulkan 实例层
		std::vector<const char*> instanceExtensions; // vulkan 实例扩展
		...
		// 有效的索引从 0 开始，因此用特殊值 VK_QUEUE_FAMILY_IGNORED（即 UINT32_MAX）作为队列族索引的默认值
		...
		std::vector <VkImage> swapchainImages; // VkImage 表示一段设备内存上的图像数据
		std::vector <VkImageView> swapchainImageViews; // VkImageView 指定图像的使用方式
		VkSwapchainCreateInfoKHR swapchainCreateInfo = {}; // 保存交换链的创建信息，以便重建交换链
		std::vector<std::function<void()>> callbacks_createSwapchain;  // 用回调统一管理依赖交换链的资源
		...
		// 该函数被 DeterminePhysicalDevice(...) 调用，用于检查物理设备是否满足所需的队列族类型，并把对应索引写回 queueFamilyIndices
		result_t GetQueueFamilyIndices(VkPhysicalDevice physicalDevice, bool enableGraphicsQueue, bool enableComputeQueue, uint32_t(&queueFamilyIndices)[3]) {
			...
			for (uint32_t i = 0; i < queueFamilyCount; i++) {
				...
				if (supportGraphics && supportCompute) {
					...
					// 除非 ig 和 ic 已经取得且相同，否则用 i 覆写，尽量保证它们相同
					if (ig != ic || ig == VK_QUEUE_FAMILY_IGNORED)
						ig = ic = i;
					// 如果不需要呈现，那么已经可以直接 break
					if (!surface)
						break;
				}
			...
			// 若任何需要被获取的队列族索引仍未取得，则函数执行失败
			if (ig == VK_QUEUE_FAMILY_IGNORED && enableGraphicsQueue ||
				ip == VK_QUEUE_FAMILY_IGNORED && surface ||
				ic == VK_QUEUE_FAMILY_IGNORED && enableComputeQueue)
				return VK_RESULT_MAX_ENUM;
			// 函数执行成功时，将所取得的队列族索引写入成员变量
		...
		// 该函数用于向 instanceLayers 或 instanceExtensions 容器中添加字符串指针，并确保不重复
		static void AddLayerOrExtension(std::vector<const char*>& container, const char* name) {
			for (auto& i : container)
				if (!strcmp(name, i)) // strcmp(...) 在字符串匹配时返回 0
					return;           // 如果层/扩展名称已在容器中，直接返回
	...
	public:
		...
		result_t SetSurfaceFormat(VkSurfaceFormatKHR surfaceFormat) {
			...
			// 如果没有符合的格式，正好有一个语义匹配的错误码
			if (!formatIsAvailable)
				return VK_ERROR_FORMAT_NOT_SUPPORTED;
			// 如果交换链已存在，则调用 RecreateSwapchain() 重建交换链
			if (swapchain)
		...
		result_t CreateSwapchain(bool limitFrameRate = true, VkSwapchainCreateFlagsKHR flags = 0) {
			...
			if (!swapchainCreateInfo.imageFormat)
				...
				if (SetSurfaceFormat({ VK_FORMAT_R8G8B8A8_UNORM, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR }) &&
					...
					// 如果找不到上述格式与色彩空间的组合，那就只能有什么用什么，采用 availableSurfaceFormats 中的第一组
		...
		// 该函数用于获取交换链图像索引到 currentImageIndex，并在需要重建交换链时调用 RecreateSwapchain()
			...
			while (VkResult result = vkAcquireNextImageKHR(device, swapchain, UINT64_MAX, semaphore_imageIsAvailable, VK_NULL_HANDLE, &currentImageIndex))
				...
				case VK_ERROR_OUT_OF_DATE_KHR:
					if (VkResult result = RecreateSwapchain())
						return result;
					break; // 注意重建交换链后仍需要重新获取图像，通过 break 回到 while 条件判断处
				default:
		...
		// 该函数用于指定所用物理设备，并调用 GetQueueFamilyIndices(...) 取得队列族索引
		result_t DeterminePhysicalDevice(uint32_t deviceIndex = 0, bool enableGraphicsQueue = true, bool enableComputeQueue = true) {
			...
			if (ig == VK_QUEUE_FAMILY_IGNORED && enableGraphicsQueue ||
				...
				// 若 GetQueueFamilyIndices(...) 返回 VK_SUCCESS 或 VK_RESULT_MAX_ENUM，说明已经得到结论
				// 说明对所需队列族索引已经有结论，保存结果到 queueFamilyIndexCombinations[deviceIndex]
			...
			// 若以上两个 if 分支都不执行，则说明所需队列族索引都已取得，直接从缓存中取回
			else {
	...
	class fence {
		VkFence handle = VK_NULL_HANDLE; //handle的概念是对外暴露的一个可操作接口
	public:
		...
		result_t Status() const {
			...
			if (result < 0) // vkGetFenceStatus(...) 成功时有两种结果，所以不能仅仅判断 result 是否非 0
	...
	public:
		...
		//鍥犻噴鏀惧懡浠ょ紦鍐插尯鐨勫嚱鏁拌鎴戝畾涔夊湪灏佽鍛戒护姹犵殑commandPool绫讳腑锛屾病鏋愭瀯鍣?
		//Getter
		DefineHandleTypeOperator;
		DefineAddressFunction;
		//Const Function
	...
	// deviceMemory 封装一段绑定到 Vulkan 资源上的显存。
	// 这里先实现最小功能：分配、映射、写入、释放。
	class deviceMemory {
	...
	public:
		...
		~deviceMemory() { DestroyHandleBy(vkFreeMemory); }
		DefineHandleTypeOperator;
		DefineAddressFunction;
		...
		void Write(const void* pData, size_t size, VkDeviceSize offset = 0) const {
			if (void* pMapped = Map(offset, size)) {
				memcpy(pMapped, pData, size);
				Unmap();
			}
		}
		result_t Create(VkMemoryAllocateInfo& allocateInfo) {
	...
	// buffer 封装 VkBuffer，本身只描述用途和大小；
	// 真正的数据存储在单独分配并绑定的 deviceMemory 中。
	class buffer {
	...
	public:
		...
		VkMemoryRequirements MemoryRequirements() const {
			...
			vkGetBufferMemoryRequirements(graphicsBase::Base().Device(), handle, &requirements);
			return requirements;
		}
		result_t BindMemory(VkDeviceMemory memory, VkDeviceSize offset = 0) const {
			VkResult result = vkBindBufferMemory(graphicsBase::Base().Device(), handle, memory, offset);
			if (result)
		...
		result_t Create(VkDeviceSize size, VkBufferUsageFlags usage, VkBufferCreateFlags flags = 0) {
